...
<ul>
<li>- CMSMS™ utilise un système d\'autorisations (permissions) nominatives. L\'accès à ces permissions détermine la capacité pour les utilisateurs de réaliser différentes fonctions dans la console dans la console d\'administration.</li>
...
<li>- Les comptes utilisateurs "Admin" peuvent être membres de zéro ou de plusieurs groupes. Il peut être possible pour un compte utilisateur qui n\'est  membre d\'auncun groupe d\'effectuer, malgré tout, différentes actions <em>(lire le paragraphe sur les propriétaires et les rédacteurs supplémentaires dans les aides du Gestionnaire des contenus et de Gestion du design)</em>. Le premier compte utilisateur <em>(UID == 1)</em>, qui est généralement nommé "Admin" est un compte utilisateur spécial et aura toutes les permissions.</li>
</ul>';
...
<p>L\'interface principale du module Content Manager est la liste des contenus. Elle affiche des éléments de contenu sous la forme d\'un tableau et offre des  possibilité de recherche, de navigation et de gestion rapide des éléments de contenu.
...
<p>Chaque colonne de la liste de contenu a un but particulier :</p>
   <ul>
     <li>Afficher/Masquer la colonne
      <p>Quand une page de contenu a des enfants cette colonne contiendra une icône qui permet de déployer la liste pour montrer les pages enfants, ou de masquer la liste des pages enfants. L\'état, dans lequel les pages affichées et donc regroupées, est enregistré. De sorte que lorsque vous reviendrez dans le gestionnaire de contenu de l\'état déployé/contracté de vos pages restera le même.</p>
     </li>
 ...
 <p>Certains types d\'éléments de contenu (tels que Séparateur et En-tête de section) ne nécessitent que très peu d\'informations, donc le nombre d\'onglets et de propriétés affichées seront réduites par rapport aux autres éléments de contenu.</p>
  ...
  <ul>
    ...
    <li>Options/Alias de page <!--/*Alias-->
      <p>L\'alias de page est une chaîne qui identifie de manière unique un élément de contenu. Il est généralement plus facile à retenir que l\'identifiant numérique ID. L\'alias est utilisé dans de nombreux endroits lors de la création de site Web CMS Made Simple. Il peut être utilisé pour créer des liens vers des éléments de contenu, construire une navigation spécialisée, ou pour des comportement relatifs à d\'autres modules, en indiquant dans quel élément de contenu, ils doivent afficher les données.</p>
    ...
    <li>Principal/Type de contenu <!--/*Content-->
      ...
      <p>Si l\'éditeur WYSIWYG est activé pour ce bloc de contenu et si l\'utilisateur a sélectionné un éditeur WYSIWYG dans "Préférences/Mon compte/Préférences utilisateur", un éditeur WYSIWYG s\'affichera. Les WYSIWYG ont des capacités différentes pour formater le texte. En outre, la plupart des éditeurs WYSIWYG permettent l\'insertion d\'images, la création des liens vers d\'autres éléments de contenu dans votre site Web.</p>
    </li>
    ...
    <li>Options/Image <!--/*Image-->
		<p>Cette propriété permet d\'associer une image, précédemment uploadée, avec cet élément de contenu. Les éditeurs peuvent sélectionner un fichier image à partir du dossier uploads/images. Cette image peut être affichée sur la page HTML généré (le cas échéant), ou utilisée lors de la construction du menu de navigation.</p>     
       <p>Les utilisateurs avec des permissions restreintes peuvent ne pas avoir la possibilité de régler ou spécifier cette propriété.</p>
    </li>
    ...
    <li>Logique/Balises Smarty spécifiques  <!--/*Page Data-->
        <p>Cette propriété est principalement utilisée pour l\'insertion de données, ou de la logique Smarty pour une utilisation par le gabarit de la page. C\'est un champ destiné à être utilisé dans des gabarits flexibles avec un comportement dynamique.</p>
        <p>Les utilisateurs avec des permissions restreintes peuvent ne pas avoir la possibilité de régler ou spécifier cette propriété.</p>
    </li>
...
<p>Chaque page de contenu HTML est associée à un design pour déterminer quelles feuilles de styles doivent être utilisés, et leur ordre. Par ailleurs, chaque page de contenu HTML est associé à un gabarit, ce gabarit n\'a pas besoin d\'être associé à un design.</p>
<p>Les gabarits et feuilles de style n\'ont pas besoin d\'être associé à un design, toutefois, seuls les gabarits et les feuilles de style qui sont associés à un design sont exportables avec le design.</p>

<h3>Quels sont les gabarits types (modèles)?  <!--/*What are Template Types ?--></h3>
  <p>Les gabarit types (modèles) permettent d\'organiser les gabarits de manière souple. Certaines fonctionnalités peuvent afficher des listes de gabarits qui sont d\'un certain type afin d\'en faciliter la sélection. Par exemple, le contenu édition de la page affiche une liste de gabarits "de page". (le gabarit "Left simple navigation + 1 column" est donc un gabarit de type "Core::Page").</p>
...
<h3>Gestion des gabarits <!--/*Managing Templates--></h3>
 ...
 <p>Le menu "Option" offre la possibilité aussi de créer un nouveau gabarit <em>(selon les permissions)</em>.</p>
   ...
   <ul>
     ...
     <li>Design :  <!--/*Design:-->
       <p>Cette colonne indique le(s) design(s) auquel le gabarit est associé (le cas échéant). Si le gabarit est associé à plusieurs designs une info-bulle affiche une liste des premiers designs associés.</p>
     </li>
     <li>Défaut	: <!--/*Default:-->
...
<h3>Gestion des feuilles de style <!--/*Managing Stylesheets--></h3>
  ...
  <p>Un menu déroulant "Option/Filtre" offre la possibilité de basculer entre les lignes des feuilles de style qui correspondent au filtre sélectionné, si plus d\'une ligne feuilles de style correspondante est affichée. La boîte de dialogue de filtre permet de filtrer et de trier les feuilles de style affichées par un certain nombre de critères, ainsi que de changer le nombre de pages (Limite), et le tri des feuilles de style.</p>
...
<p>Ce texte identifie le design. Le système va générer une erreur lors de la sauvegarde du design si le nom est déjà utilisé avec un autre design.</p>
      ...
      <li>Gabarits <!--/*Templates:-->
        <p>Cet onglet permet de sélectionner les différents gabarits à associer au design.Vous avez la possibilité d\'utiliser le glisser-déplacer pour ajouter, ordonner des gabarits dans la liste "Gabarits attachés". Actuellement l\'ordre des gabarits dans la liste de gabarits attachés n\'est pas importante.</p>
      </li>
  ...
  <h4>Importer des designs  <!--/*Importing Designs--></h4>
     ...
     <ul>
      <li>Étape 1 : Uploader le fichier  <!--/*Step 1: Upload the file:-->
        <p>Cette étape gère l\'upload quand l\'utilisateur a sélectionné un fichier XML et valider son contenu. Cette étape est liée à la configuration PHP sur les limites de taille fichier, de mémoire et de temps de traitement. Vous devrez peut-être vérifier et augmenter ces limites pour les gros fichiers XML.</p>
...
$lang['help_myaccount_ignoremodules'] = 'Si les notifications d\'administration sont activés, vous pouvez choisir d\'ignorer les notifications provenant de certains modules';
...
$lang['settings_mailprefs_mailer'] = 'Ce choix détermine comment CMSMS™ va envoyer les mails. Utiliser la fonction PHP <a href="http://www.cmsmadesimple.org/downloads/" target="_blank">mail</a>, Sendmail,  ou communiquer directement avec un serveur SMTP.<br/><br/>Les options mail et sendmail devraient fonctionner sur la plupart des serveurs Gnu-Linux, mais ne fonctionneront sûrement pas sur la plupart serveurs Windows.<br/><br/>L\'option "sendmail" devrait fonctionner sur la plupart des serveurs Gnu-Linux. Cependant, il ne fonctionnera surement pas sur des serveurs partagés.<br/><br/>L\'option SMTP requiert des informations de configuration de votre serveur.';
...
$lang['siteprefs_backendwysiwyg'] = 'Sélectionnez l\'éditeur WYSIWYG pour les comptes utilisateur d\'administration nouvellement créées. Les utilisateurs "Admin" pourront ensuite choisir leur éditeur WYSIWYG préféré partir dans le panneau de préférences de l\'utilisateur.';
